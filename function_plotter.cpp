...
void iMouse(int button, int state, int mx, int my)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
	{
		//place your codes here
		if(mode==0){
			if((mx>=400 && mx<=528) && (my>=200&&my<=328))
				mode=4;
			...
			/*else if((mx>=700 && mx<=844)&&(my>=200 && my<= 240)){
				mode=3;
				primlength=0;
				current=1;
				primary[1][0]='\0';
		...
		/*else if(mode==3){
			if((mx>=1040 && mx<=1072)&&(my>=newy1 && my<=newy2)){
				if(current<3 && current>0 && (current%2==0) && primlength){
					current++;
					newy1=newy1-100;
					newy2=newy2-100;
					calcuflag=0;
...
void initial_Window(void)
{
	...
	if(mode==0){
		...
		//iShowBMP(700,200,"parametric.bmp");
	...
	//else if(mode==3) parametric_draw();
	...
	iFilledRectangle(12,window_sizeY-440,688,436);
	iSetColor(255,255,255);
	iFilledRectangle(15,window_sizeY-437,682,430);

	iSetColor(0,0,0);
	iText(window_sizeX-250,440,"Enter function here:",GLUT_BITMAP_9_BY_15);*/
}
...
void cartesian_draw()
{
	...
	iSetColor(0,0,255);
	/*my=window_sizeY-637;
	for(mx=15;mx<=609;mx+=10){
		iLine(mx,window_sizeY-617,mx,0);
	}*/

	/*iSetColor(100,255,250);
	
	iFilledRectangle(0,0,window_sizeX,window_sizeY);*/
	iShowBMP(0,0,"rene_descartes.bmp");
	iSetColor(0,0,0);
	...
	//iSetColor(255,255,255);
	fxpos=399;
	//iShowBMP(745,399,"y=f(x).bmp");
	//iFilledRectangle(780,400,180,30);
	if(current<3){
...
void polar_draw()
{
	...
	if(calcuflag){
		...
		for(arrayi=1;arrayi<totalarray;++arrayi){
			...
			/*p1=finalx[func_serial][arrayi-1];
			p2=finaly[func_serial][arrayi-1];
			p3=finalx[func_serial][1];
			p4=finaly[func_serial][1];
			if(mapvalue[func_serial][arrayi-1]==0 && mapvalue[func_serial][1]==0){
				iLine(p1,p2,p3,p4);
			}*/
